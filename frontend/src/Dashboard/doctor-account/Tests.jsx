    ...
    <>
      <table className="w-full text-left text-sm text-gray-500">
        ...
        <tbody>
          {tests?.map((item) => (
            <tr key={item._id}>
              <td className="px-6 py-4 text-[9px] lg:text-sm">{item?.labTest?.name}</td>
              <td className="px-6 py-4 text-[9px] lg:text-sm">{item?.user?.name}</td>
      ...
      <SlideReport onClose={() => setShowSlideReport(false)}>
        {/* Render content inside the slide-over */}

        {/* <h1>{tests && tests.labTest && tests.labTest.map((test)=>(
          <div key={labTest.id}>
            <p>{test.name}</p>
          </div>
        ))}</h1> */}
